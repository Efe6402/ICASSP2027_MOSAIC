...
%APPLY_GRAPH_F1_THRESHOLDS Apply frozen validation-selected relative taus.
defs={'supra','all','all';'within','within','within'; ...
    'cross','crossnode_crossview','crossnode_crossview';'copy','copy','copy'};
m=struct();
for j=1:size(defs,1)
    label=defs{j,1}; z=one(A,truth,defs{j,2},defs{j,3},tau.(label));
    m.(label)=z.f1; m.([label,'_precision'])=z.precision;
    m.([label,'_recall'])=z.recall;
end
m.component_average=mean([m.within,m.cross,m.copy]);
end
