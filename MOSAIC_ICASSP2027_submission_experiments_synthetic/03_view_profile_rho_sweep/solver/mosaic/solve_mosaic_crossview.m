...
best_any_J = inf; best_any = 0;
best_converged_J = inf; best_converged = 0;
for s = 1:opts.n_restarts
    [Theta,Gamma,B] = initialize(problem,opts,s);
    records{s} = one_restart(Theta,Gamma,B,problem,opts,s);
    if isfinite(records{s}.objective) && records{s}.objective < best_any_J
        best_any_J = records{s}.objective; best_any = s;
    end
    if strcmp(records{s}.status,'converged') && ...
            records{s}.objective < best_converged_J
        best_converged_J = records{s}.objective; best_converged = s;
    end
    if opts.verbose
...
if best_converged>0
    best=best_converged; selection_policy='lowest_objective_converged_restart';
else
    best=best_any; selection_policy='fallback_lowest_objective_nonconverged_restart';
end
if best == 0, error('Every MOSAIC restart failed.'); end
result = records{best};
result.best_restart = best;
result.restart_selection_policy=selection_policy;
result.all_restarts = records;
result.options = opts;
result.cost_scale = problem.cost_scale;
% Preserve derived costs/metadata without duplicating large signal matrices
% inside every saved candidate fit.
if isfield(observed,'Y'), observed=rmfield(observed,'Y'); end
...
for it=1:o.max_iter
    ...
    if o.verbose_iterations && (it==1 || mod(it,o.print_every)==0)
        fprintf(['    restart=%d iter=%d/%d J=% .6e relJ=%.3e ', ...
            'rTheta=%.3e rGamma=%.3e rB=%.3e elapsed=%.1fs\n'], ...
            restart,it,o.max_iter,J,relJ,rth,rga,rB,toc(restart_clock));
    end
...
rec.stationarity_max=max([rth,rga,rB]);
if numel(hist)>=2
    rec.final_relative_objective_change=abs(hist(end)-hist(end-1))/ ...
        max(1,abs(hist(end-1)));
else
    rec.final_relative_objective_change=NaN;
end
...
d=struct('operation','solve','seed',1,'n_restarts',8,'max_iter',1200, ...
...
d.verbose_iterations=false; d.print_every=100;
o=merge(d,o);
...
if o.print_every<1||o.print_every~=round(o.print_every)
    error('print_every must be a positive integer.');
end
end
